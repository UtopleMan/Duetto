...
public sealed class S3FileSystemProviderContractTests
{
    private static (S3FileSystemProvider Provider, FakeS3ClientAdapter Adapter) Make(string configuredBucket = "")
    {
        var adapter = new FakeS3ClientAdapter("duetto");
        var info = new S3ConnectionInfo(Id: "c1", Name: "n", Bucket: configuredBucket);
    ...
    {
        var caps = S3FileSystemProvider.S3Capabilities;
    ...
    [Fact]
    public void List_root_returns_buckets()
    {
        var (provider, _) = Make();
    ...
    [Fact]
    public void List_root_in_single_bucket_mode_lists_only_that_bucket()
    {
        var (provider, _) = Make(configuredBucket: "duetto");
        var root = provider.List("/");
        Assert.Single(root);
        Assert.Equal("duetto", root[0].Name);
    }
    ...
    {
        var (provider, _) = Make();
        var payload = Encoding.UTF8.GetBytes("s3 provider bytes");
    ...
    [Fact]
    public void Delete_file_removes_only_that_object()
    {
        var (provider, adapter) = Make();
