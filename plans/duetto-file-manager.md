# Duetto — two-pane file manager (Avalonia / C#)

Build Duetto v1 from the Claude design spec "Duetto File Manager.dc.html" (project
...
- Solution name **Duetto**, app name **Duetto**. Repo root `/Users/dude/Sources/UtopleMan/duetto`, git-initialized in Phase 1.
...
- Deliverables: `dotnet publish` self-contained single-file per RID (osx-arm64, win-x64, linux-x64) **plus a macOS `Duetto.app` bundle**.
...
└─ tests/Duetto.Tests/       xunit; Core unit tests + Avalonia.Headless UI tests
```
...
- [x] Create solution with `src/Duetto.Core` (classlib), `src/Duetto` (Avalonia app), `tests/Duetto.Tests` (xunit, refs both)
...
- [x] Bundle IBM Plex Mono Regular/Medium/SemiBold/Bold TTFs in `src/Duetto/Assets/Fonts`, register as embedded font family
...
- `dotnet run --project src/Duetto -- --smoke` renders window then exits 0 within ~1 s (no `timeout` cmd on this Mac — background + `kill -0` poll)
...
- Fonts: IBM Plex Mono 4 weights from google/fonts (OFL), resource key `MonoFont` in `App.axaml` (`avares://Duetto/Assets/Fonts#IBM Plex Mono`).
- `Duetto.csproj` uses compiled bindings by default (`AvaloniaUseCompiledBindingsByDefault=true`).

## Phase 2: Core domain (Duetto.Core, test-first)
Status: Complete
...
### Verification Plan
- `dotnet test --filter "FullyQualifiedName~Duetto.Tests.Core"` → all pass
- Trash test on macOS: create temp file, `TrashService.Trash`, assert gone from source + present in `~/.Trash`

### Phase Summary
Done 2026-07-26. 40/40 tests green (`dotnet test --filter FullyQualifiedName~Duetto.Tests.Core`). API surface for the UI phases:
...
- `dotnet run --project src/Duetto -- --smoke` exits 0
- Manual: `dotnet run --project src/Duetto`, navigate repo root, check 27px rows, active-pane tint follows Tab
...
- `TransferSession.StateOf(path)` added to Duetto.Core for per-row percent badges.
...
- [x] `scripts/make-app-bundle.sh` builds `dist/Duetto.app` (Info.plist, icns generated from simple blue-square mark, osx-arm64 binary), plus zips of all three binaries in `dist/`
- [x] Headless render test per chrome (instantiate each chrome, assert key elements present)

### Verification Plan
- `dotnet run --project src/Duetto -- --chrome win --smoke` / `--chrome gnome --smoke` / `--chrome mac --smoke` all exit 0
- `./scripts/publish-all.sh` succeeds; `file dist/*/Duetto*` shows Mach-O arm64 / PE32+ x86-64 / ELF x86-64; `dist/osx-arm64/Duetto --smoke` exits 0; `plutil -lint dist/Duetto.app/Contents/Info.plist` OK; `open dist/Duetto.app` (needs unlocked screen)
- `dotnet test` full suite green

### Phase Summary
Done 2026-07-26. 72/72 tests, all three chrome smokes exit 0, three publishes + Duetto.app built (zips ~40 MB each in dist/). Implementation notes:
...
### Phase Summary
Done 2026-07-26. Added `--screenshot <path>` mode (headless Skia frame capture via `HeadlessWindowExtensions.CaptureRenderedFrame`) and compared all three chromes against the spec tokens — layouts, palette, mono paths, tinted active bar, chips, rail, dark GNOME strip, and command prompt all match. Fixed disabled toolbar buttons painting Fluent's gray box (ContentPresenter background override). Edge cases covered by tests: empty dir, vanished dir, 3000-file dir loads < 5 s, 120-char names. Final state: **76/76 tests green**, three chrome smokes exit 0, publishes rebuilt, `dist/osx-arm64/Duetto --smoke` exits 0. Note: 3000-file test creates files each run (~1 s); pane virtualization is Avalonia's default VirtualizingStackPanel. Visual deltas accepted vs design: mac toolbar is a full-width strip (design floats it on the desk), native traffic lights absent in headless captures only.

## Final Recap
Duetto v1 is complete: an orthodox two-pane file manager (Avalonia 11.3.18, net10.0, CommunityToolkit.Mvvm 8.4.2) matching the Claude design spec. Solution `Duetto.slnx`: `src/Duetto.Core` (pure domain: lister/sorter/formats, transfer engine with pause/cancel/auto-skip, per-OS trash, streaming search, shell runner), `src/Duetto` (Avalonia app: pane views, progress strip, command bar + output drawer, search results overlay, three chromes), `tests/Duetto.Tests` (76 tests: Core xunit + Avalonia.Headless UI, Skia-backed).
...
2. Verify: `dotnet test` (expect 76/76) and `for c in win mac gnome; do dotnet run --project src/Duetto -- --chrome $c --smoke; done` (all exit 0).
3. Build binaries: `./scripts/publish-all.sh` → self-contained single-file executables in `dist/osx-arm64/Duetto`, `dist/win-x64/Duetto.exe`, `dist/linux-x64/Duetto`, plus `dist/duetto-<rid>.zip` (~40 MB each).
4. macOS bundle: `./scripts/make-app-bundle.sh` → `dist/Duetto.app` (generates icns via scripts/make-icon.py + sips/iconutil, ad-hoc codesigned). Launch with `open dist/Duetto.app`.
5. Distribute the zips / Duetto.app directly — no installer, no runtime prerequisite on target machines. Windows/Linux binaries are cross-compiled and untested on real target OSes; run the exe once on a real Windows/Linux box before shipping broadly.
...
- Focus robustness (root causes found via `DUETTO_FOCUS_LOG` instrumentation): macOS wipes focus at window activation, and every watcher reload used to kill the focused row container. Fixes: `Activated` handler refocuses when focus is null; `PaneViewModel.Reloaded` event + `PaneView.OnVmReloaded` restores focus to the active pane only when focus died; all navigation (Enter/Backspace/Tab/double-click) refocuses via deferred `RefocusActiveList`.
